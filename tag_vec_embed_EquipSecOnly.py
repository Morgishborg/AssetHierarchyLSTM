...
df_tags.columns = ['X', 'y']
# Function to remove first three sections (to get only the equipment relevant sections of the tags)
def remove_first_three_sections(s):
    sections = s.split('-')
    # Keep all sections after the first three
    remaining_sections = sections[3:]
    return '-'.join(remaining_sections)
# Apply the function to the DataFrame column
df_tags['X'] = df_tags['X'].apply(remove_first_three_sections)
df_tags['y'] = df_tags['y'].apply(remove_first_three_sections)
...
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy']) # standard loss: loss='categorical_crossentropy' #loss=custom_loss
